...
-- MySQL-compatible version (handles index creation safely)
-- =====================================================================
...
-- Check and create idx_promotions_expires_at
SELECT COUNT(*) INTO @idx_exists FROM information_schema.statistics 
WHERE table_schema = @db_name 
AND table_name = 'waiting_list_promotions' 
AND index_name = 'idx_promotions_expires_at';

SET @sql = IF(@idx_exists = 0, 
    'CREATE INDEX idx_promotions_expires_at ON waiting_list_promotions(expires_at)',
    'SELECT ''Index idx_promotions_expires_at already exists'' AS message');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Check and create idx_promotions_status
SELECT COUNT(*) INTO @idx_exists FROM information_schema.statistics 
WHERE table_schema = @db_name 
AND table_name = 'waiting_list_promotions' 
AND index_name = 'idx_promotions_status';

SET @sql = IF(@idx_exists = 0, 
    'CREATE INDEX idx_promotions_status ON waiting_list_promotions(status)',
    'SELECT ''Index idx_promotions_status already exists'' AS message');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
