...
When asked to perform a code review, provide the **most detailed review possible**:

**Check EVERY method for:**
1. **NPE risks**: Every `.toString()`, `.equals()`, `.substring()`, `.format()`, `.length()` call on potentially null objects
2. **Method chains**: `a.getB().getC()` - verify each step can't return null
3. **Format specifiers**: `%d` must receive numbers, `%s` for strings - check all `String.format()` calls
4. **Resource leaks**: Streams, connections, file handles must be closed (use try-with-resources)
5. **Dead code**: Unreachable branches, always-true/false conditions
6. **Unused imports**: Flag all unused imports for removal
7. **Exception handling**: Catch blocks that swallow exceptions silently
8. **SQL/JPQL**: Native queries with incorrect parameter syntax

**Output format:**
```
| Line | Issue | Severity | Fix |
|------|-------|----------|-----|
| 123  | NPE risk on token.substring() | HIGH | Add null check |
```
...
### Claude Output Rules
When displaying API request/response JSON output:
- Always pretty-print JSON with indentation for readability
- Example: `curl -s "URL" | python -c "import sys,json; print(json.dumps(json.load(sys.stdin), indent=2))" | head -50`
...
### Registration Limit Tests (Once Per Degree)
Students can only present **once during their entire degree** (MSc or PhD). This is enforced by `registration.max_approved = 1`.

| User | Rule | Test |
|------|------|------|
| MSc | 1 presentation per degree | After one slot is approved, user cannot register for any other slot (including future semesters) |
| PhD | 1 presentation per degree | After one slot is approved, user cannot register for any other slot (including future semesters) |

**Note:** Past registrations (from previous semesters) count towards this limit. A user who presented last semester cannot present again.
...
-- Table: slot_presenter_sessions (tracks attendance per presenter per slot)
CREATE TABLE `slot_presenter_sessions` (
  `id` bigint NOT NULL AUTO_INCREMENT,
  `slot_id` bigint NOT NULL,
  `presenter_username` varchar(50) NOT NULL,
  `presenter_first_name` varchar(100) DEFAULT NULL,
  `presenter_last_name` varchar(100) DEFAULT NULL,
  `presenter_full_name` varchar(200) DEFAULT NULL,
  `opened_at` datetime DEFAULT NULL,
  `closes_at` datetime DEFAULT NULL,
  `status` enum('NOT_OPENED','OPEN','CLOSED') NOT NULL DEFAULT 'NOT_OPENED',
  `session_id` bigint DEFAULT NULL,
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` datetime DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `unique_slot_presenter` (`slot_id`,`presenter_username`),
  CONSTRAINT `fk_slot_presenter_sessions_slot` FOREIGN KEY (`slot_id`) REFERENCES `slots` (`slot_id`) ON DELETE CASCADE,
  CONSTRAINT `fk_slot_presenter_sessions_presenter` FOREIGN KEY (`presenter_username`) REFERENCES `users` (`bgu_username`) ON DELETE CASCADE,
  CONSTRAINT `fk_slot_presenter_sessions_session` FOREIGN KEY (`session_id`) REFERENCES `sessions` (`session_id`) ON DELETE SET NULL
);
