...
- Two devices (emulators or phones)
- Device 1: MSc user (talguest3 / kbm7Xzfk)
- Check supervisor email access (talbnwork@gmail.com)

**Cleanup Command** (run before test):
```sql
DELETE FROM waiting_list WHERE presenter_username IN ('talguest3','talguest4','benariet','talguest2','amarrev');
DELETE FROM slot_registration WHERE presenter_username IN ('talguest3','talguest4','benariet','talguest2','amarrev');
UPDATE slots SET status = 'FREE' WHERE slot_date >= CURDATE();
```

| Step | Device | Action | Expected Result |
|------|--------|--------|-----------------|
| 1 | Device 1 | Open app, login as `talguest3` / `kbm7Xzfk` | Home screen shows available slots |
| 2 | Device 1 | Find a GREEN slot (Jan 12 or later) | Slot card shows "Register Now" button |
| 3 | Device 1 | Tap "Register Now" button | Dialog shows registration form |
| 4 | Device 1 | Fill topic, select supervisor, tap Submit | Toast: "Registration submitted" |
| 5 | Device 1 | View the same slot | Shows "Pending Approval" status, YELLOW color |
...
| 9 | Device 1 | Check notification tray | FCM: "Registration Approved!" |
| 10 | Device 1 | Pull down to refresh slot list | Slot shows "Approved" status, RED color |
...
**Prerequisites**:
- Two devices
- Device 1: PhD user (talguest2 / tc2xqVds)
- Device 2: MSc user (talguest3 / kbm7Xzfk)
...
| Step | Device | Action | Expected Result |
|------|--------|--------|-----------------|
| 1 | Device 1 | Login as `talguest2` / `tc2xqVds` (PhD) | Home screen |
| 2 | Device 1 | Find slot 234 (should be GREEN) | Shows "Register Now" button |
| 3 | Device 1 | Tap "Register Now", fill form, submit | Toast: "Registration submitted" |
| 4 | Device 1 | View slot 234 | Slot is now RED, shows "Pending Approval" |
| 5 | Device 2 | Login as `talguest3` / `kbm7Xzfk` (MSc) | Home screen |
| 6 | Device 2 | Find slot 234 | Slot is RED |
| 7 | Device 2 | Tap on slot 234 | Shows "Join Waiting List" button (NOT Register) |
| 8 | Device 2 | Tap "Join Waiting List" | Toast: "Added to waiting list" |
| 9 | Device 2 | View slot 234 again | Shows "Cancel Waiting List" button, position #1 |

**Verification Query**:
```sql
SELECT * FROM waiting_list WHERE slot_id = 234;
-- Should show talguest3 at position 1
```
...
**Prerequisites**:
- Two devices
- Device 1: talguest3 (will register and cancel)
- Device 2: amarrev (will join waiting list)

**Setup Command** (create a full slot with 2 MSc):
```sql
...
VALUES
(234, 'talguest4', 'MSc', 'APPROVED', 'Dr. Test', 'talbnwork@gmail.com'),
(234, 'benariet', 'PhD', 'APPROVED', 'Dr. Test', 'talbnwork@gmail.com');
UPDATE slots SET status = 'SEMI' WHERE slot_id = 234;
-- Note: Use testmsc1/testmsc2 if you need more users
```

| Step | Device | Action | Expected Result |
|------|--------|--------|-----------------|
| 1 | Device 1 | Login as `talguest3` / `kbm7Xzfk` | Home screen |
| 2 | Device 1 | Find slot 234 (YELLOW - 2 MSc) | Shows "Register Now" button |
| 3 | Device 1 | Register for slot 234 | Slot becomes RED (FULL with 3 MSc) |
| 4 | Device 2 | Login as `amarrev` / `Revital1990%` | Home screen |
| 5 | Device 2 | Find slot 234 (RED - FULL) | Shows "Join Waiting List" button |
| 6 | Device 2 | Tap "Join Waiting List" | Toast: "Position 1 on waiting list" |
| 7 | Device 1 | Find slot 234, tap to view details | Shows "Cancel Registration" button |
| 8 | Device 1 | Tap "Cancel Registration", confirm | Toast: "Registration cancelled" |
| 9 | Device 2 | Check notification tray (within 5 sec) | FCM: "Slot Available!" |
| 10 | Device 2 | Pull down to refresh | Slot 234 shows "Pending Approval" (promoted!) |

**Verification Query**:
```sql
-- amarrev should now be in slot_registration, not waiting_list
SELECT * FROM slot_registration WHERE slot_id = 234 AND presenter_username = 'amarrev';
SELECT * FROM waiting_list WHERE slot_id = 234;  -- Should be empty or not have amarrev
```
...
**Prerequisites**:
- One device with talguest3
- Access to supervisor email

| Step | Device | Action | Expected Result |
|------|--------|--------|-----------------|
| 1 | Device 1 | Login as `talguest3` / `kbm7Xzfk` | Home screen |
| 2 | Device 1 | Register for an empty slot (e.g., slot 235) | Shows "Pending Approval" |
| 3 | Terminal | Get approval token | `SELECT approval_token FROM slot_registration WHERE presenter_username='talguest3' AND slot_id=235;` |
...
| 6 | Device 1 | Check notification tray | FCM: "Registration Declined" with reason |
| 7 | Device 1 | Pull down to refresh | Slot 235 shows "Register Now" button again (GREEN) |
| 8 | Device 1 | Tap "Register Now" again | Can register again |
| 9 | Device 1 | Submit new registration | Shows "Pending Approval" |

**Verification Query**:
```sql
SELECT slot_id, approval_status, registered_at, supervisor_declined_reason
FROM slot_registration WHERE presenter_username = 'talguest3' AND slot_id = 235;
...
**Prerequisites**:
- Two devices (or one device + physical phone)
- Device 1: Presenter with APPROVED registration for TODAY's slot
- Device 2: Participant user
...
INSERT INTO slot_registration (slot_id, presenter_username, degree, approval_status, supervisor_name, supervisor_email)
VALUES (229, 'talguest3', 'MSc', 'APPROVED', 'Dr. Test', 'talbnwork@gmail.com')
...
| Step | Device | Action | Expected Result |
|------|--------|--------|-----------------|
| 1 | Device 1 | Login as `talguest3` / `kbm7Xzfk` (presenter) | Home screen |
| 2 | Device 1 | Find today's slot (229) | Shows "Open Attendance" button |
| 3 | Device 1 | Tap "Open Attendance" | QR code displayed with timer |
| 4 | Device 2 | Login as `talguest4` / `atpgK2zc` (participant) | Home screen |
| 5 | Device 2 | Tap "Scan QR" or participant mode | Camera opens |
| 6 | Device 2 | Point camera at Device 1's QR | Toast: "Attendance marked!" |
| 7 | Device 1 | View attendance list | talguest4 appears in list |
| 8 | Device 1 | Wait 15 min OR tap "Close Session" | Session closes |
| 9 | Device 2 | Try to scan again | Error: "Session closed" |
...
WHERE a.student_username = 'talguest4'
...
-- Slot B: 1 MSc (YELLOW)
INSERT INTO slot_registration (slot_id, presenter_username, degree, approval_status, supervisor_name, supervisor_email)
VALUES (234, 'talguest3', 'MSc', 'APPROVED', 'Dr. Test', 'talbnwork@gmail.com')
ON DUPLICATE KEY UPDATE approval_status = 'APPROVED';
UPDATE slots SET status = 'SEMI' WHERE slot_id = 234;

-- Slot C: 1 PhD (RED)
INSERT INTO slot_registration (slot_id, presenter_username, degree, approval_status, supervisor_name, supervisor_email)
VALUES (235, 'talguest2', 'PhD', 'APPROVED', 'Dr. Test', 'talbnwork@gmail.com')
ON DUPLICATE KEY UPDATE approval_status = 'APPROVED';
...
| Step | Device | Action | Expected Result |
|------|--------|--------|-----------------|
| 1 | Device | Login as `talguest4` / `atpgK2zc` | Home screen with slot list |
| 2 | Device | Find slot 233 | GREEN background gradient |
| 3 | Device | Find slot 234 | YELLOW background gradient |
| 4 | Device | Find slot 235 | RED background gradient |
| 5 | Device | Tap on GREEN slot | Shows "Register Now" button |
| 6 | Device | Tap on YELLOW slot | Shows "Register Now" (if MSc) or "Join Waiting List" (if PhD) |
| 7 | Device | Tap on RED slot | Shows "Join Waiting List" button |
...
INSERT INTO slot_registration (slot_id, presenter_username, degree, approval_status, supervisor_name, supervisor_email)
VALUES
(234, 'talguest3', 'MSc', 'APPROVED', 'Dr. Test', 'talbnwork@gmail.com'),
(234, 'talguest4', 'MSc', 'APPROVED', 'Dr. Test', 'talbnwork@gmail.com');
UPDATE slots SET status = 'SEMI' WHERE slot_id = 234;
```
...
| Step | Device | Action | Expected Result |
|------|--------|--------|-----------------|
| 1 | Device | Login as `talguest2` / `tc2xqVds` (PhD) | Home screen |
| 2 | Device | Find slot 234 (YELLOW - 2 MSc) | Shows YELLOW, "1/3 Available" |
| 3 | Device | Tap on slot 234 | Shows "Join Waiting List" button (NOT Register) |
| 4 | Device | Check message | "Not enough capacity for PhD registration" |
| 5 | Device | Tap "Join Waiting List" | Toast: "Added to waiting list" |
| 6 | Device | View slot again | Shows "Cancel Waiting List", position #1 |

**Verification Query**:
```sql
SELECT * FROM waiting_list WHERE slot_id = 234 AND presenter_username = 'talguest2';
```
